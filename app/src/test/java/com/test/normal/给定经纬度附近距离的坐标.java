...
public class 给定经纬度附近距离的坐标 {
     ...
     * 地球每度的弧长(km)
     * */
    public final double EARTH_ARC = 111.199;
     ...
     * @return 两点距离，单位km
     * */
    public double GetDistanceOne(double lon1, double lat1, double lon2,
                                        double lat2) {
        double r1 = rad(lat1);
        double r2 = rad(lon1);
        double a = rad(lat2);
     ...
     * @return 两点距离，单位km
     * */
    public double GetDistanceTwo(double lon1, double lat1, double lon2,
                                        double lat2) {
     ...
     * @return 两点距离，单位km
     * */
    public double GetDistanceThree(double lon1, double lat1,
     ...
     * @return 方位角，角度（单位：°）
     * */
    public double GetAzimuth(double lon1, double lat1, double lon2,
                                    double lat2) {
        lat1 = rad(lat1);
        lat2 = rad(lat2);
        lon1 = rad(lon1);
     ...
     * @return B的经纬度
     * */
    public String GetOtherPoint(double lon1, double lat1,
     ...
     * @return B的经纬度
     * */
    public String ConvertDistanceToLogLat(double lng1, double lat1,
                                                 double distance, double azimuth) {
        azimuth = rad(azimuth);
        // 将距离转换成经度的计算公式
    ...
    public double ConvertDistanceToLat(double lng1, double lat1,
    		double distance, double azimuth) {
		azimuth = rad(azimuth);
		// 将距离转换成经度的计算公式
		/*double lon = lng1 + (distance * Math.sin(azimuth))
    ...
    public double ConvertDistanceToLon(double lng1, double lat1,
    		double distance, double azimuth) {
		azimuth = rad(azimuth);
		// 将距离转换成经度的计算公式
		double lon = lng1 + (distance * Math.sin(azimuth))
    ...
    private double[] getSquareLatLng(double lon, double lat, double distance) {
        double latLng[] = new double[8];
        latLng[0] = ConvertDistanceToLon(lon, lat, distance, 270);
        latLng[1] = ConvertDistanceToLat(lon, lat, distance, 0);
        latLng[2] = ConvertDistanceToLon(lon, lat, distance, 90);
        latLng[3] = ConvertDistanceToLat(lon, lat, distance, 0);
        latLng[4] = ConvertDistanceToLon(lon, lat, distance, 90);
        latLng[5] = ConvertDistanceToLat(lon, lat, distance, 180);
        latLng[6] = ConvertDistanceToLon(lon, lat, distance, 270);
        latLng[7] = ConvertDistanceToLat(lon, lat, distance, 180);
        return latLng;
    }
